...
const STYLES = `
  ...
  /* ── Page turn: exit left then enter from right (Next) ── */
  @keyframes exitLeft {
  ...
  /* ── Page turn: exit right then enter from left (Prev) ── */
  @keyframes exitRight {
    0%   { opacity:1; transform:translateX(0)   }
    100% { opacity:0; transform:translateX(32px) }
  }
...
/* ─────────────────────────────────────────────────────────────
   PageContent — isolated component so only IT re-renders
   on idx change, not the whole book shell.
   Key prop forces a full remount → clean animation every time.
───────────────────────────────────────────────────────────── */
function PageContent({ spread, idx, enterClass, creativeNote }) {
    return (
        <div
            key={idx}                  /* remount on every page change */
...
export default function StoryBook({
    ...
    const [idx, setIdx] = useState(0)        // target index
    const [displayIdx, setDisplayIdx] = useState(0)        // what's actually rendered
    const [enterClass, setEnterClass] = useState('')        // animation on the incoming page
    ...
    /*
      flip() — two-phase:
      Phase 1: set idx (triggers exit anim via useEffect below)
      Phase 2: after exit duration, update displayIdx + set enter class
    */
    const flip = (dir) => {
        if (flipping) return
        ...
        setFlipping(true)
        setIdx(next)
        // after exit animation (220ms), swap content and play enter animation
        setTimeout(() => {
            setDisplayIdx(next)
            setEnterClass(dir === 'next' ? 'pg-enterRight' : 'pg-enterLeft')
            setTimeout(() => { setFlipping(false); setEnterClass('') }, 280)
        }, 220)
    }
    ...
    const jumpTo = (i) => {
        ...
        // override the next value since flip() uses idx+1/idx-1
    ...
    /* exit class while flipping — applied to the OUTGOING page */
    const exitClass = flipping
        ? (idx > displayIdx ? 'pg-exitLeft' : 'pg-exitRight')
        : ''
        ...
        <div style={{ maxWidth: 660, margin: '0 auto', padding: '40px 20px' }}>
            ...
            {creativeNote && (
                <div style={{
                    ...
                    background: '#FFF1F2', border: '1.5px dashed #FECDD3',
                    borderRadius: 12, fontSize: 13, color: '#9F1239',
    ...
    if (done && !bookOpen) return (
        <div className="anim-bookUp" style={{
            ...
            {!isOpening ? (
                ...
                <div style={{
                    fontFamily: "'DM Sans',sans-serif",
                    color: '#9CA3AF', fontSize: 15, fontStyle: 'italic',
                }}>
                    Opening the book...
                </div>
            )}
    ...
    if (bookOpen && spreads.length > 0) return (
        <div style={{ padding: '28px 12px 52px' }} className="anim-bookUp">
            <style>{STYLES}</style>
            ...
            {/* Book shell — static, never re-renders on page turn */}
            <div style={{
                ...
                {/*
          KEY is the secret weapon:
          React unmounts old PageContent and mounts new one.
          exitClass plays on outgoing, enterClass plays on incoming.
          No shared state between the two → zero flicker.
        */}
                <PageContent
                    key={displayIdx}
                    spread={spread}
                    idx={displayIdx}
                    enterClass={flipping && idx !== displayIdx ? exitClass : enterClass || 'pg-enterRight'}
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    {spreads.map((_, i) => (
                        <button
                            ...
                            onClick={() => jumpTo(i)}
                            style={{
                                width: i === displayIdx ? 20 : 8,
                                height: 8, borderRadius: 4,
                                border: 'none', padding: 0,
                                cursor: i === displayIdx ? 'default' : 'pointer',
                                background: i === displayIdx ? '#6D28D9' : '#DDD6FE',
                                transition: 'all .2s',
                            }}
                        />
                    ))}
