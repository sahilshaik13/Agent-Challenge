...
const BOOK = [
    ['#7C3AED', '#7C3AED', '#7C3AED', '#7C3AED', P, '#A78BFA', '#A78BFA', '#A78BFA'],
    ['#7C3AED', '#FDE68A', '#7C3AED', '#7C3AED', P, '#A78BFA', '#FDE68A', '#A78BFA'],
    ['#7C3AED', '#7C3AED', '#7C3AED', '#7C3AED', P, '#A78BFA', '#A78BFA', '#A78BFA'],
    ['#7C3AED', '#7C3AED', '#7C3AED', '#7C3AED', P, '#A78BFA', '#A78BFA', '#A78BFA'],
    ['#7C3AED', '#7C3AED', '#7C3AED', '#7C3AED', P, '#A78BFA', '#A78BFA', '#A78BFA'],
    ['#5B21B6', '#5B21B6', '#5B21B6', '#5B21B6', P, '#8B5CF6', '#8B5CF6', '#8B5CF6'],
    ['#5B21B6', '#5B21B6', '#5B21B6', '#5B21B6', '#4C1D95', '#8B5CF6', '#8B5CF6', '#8B5CF6'],
    [P, '#5B21B6', '#5B21B6', '#4C1D95', '#8B5CF6', '#8B5CF6', '#8B5CF6', P],
]
...
/* ─── GROUP SEGMENTS INTO SPREADS ─── */
function toSpreads(segments) {
    const out = []
    let texts = []
    for (const s of segments) {
        if (s.type === 'text') { texts.push(s.content) }
        else if (s.type === 'image' || s.type === 'image_loading') {
            out.push({ text: texts.join(''), image: s.type === 'image' ? s.url : null, loading: s.type === 'image_loading', id: s.id })
            texts = []
        }
    }
...
const STYLES = `
  @keyframes bookUp    { from{opacity:0;transform:translateY(48px) scale(.94)} to{opacity:1;transform:none} }
  @keyframes flipClose { 0%{transform:perspective(2400px) rotateY(0);opacity:1} 60%{opacity:1} 100%{transform:perspective(2400px) rotateY(-172deg);opacity:0} }
  @keyframes pagesIn   { 0%,58%{opacity:0} 100%{opacity:1} }
  @keyframes pageOut   { from{opacity:1;transform:perspective(1400px) rotateY(0) scale(1)} to{opacity:0;transform:perspective(1400px) rotateY(-65deg) scale(.97)} }
  @keyframes pageIn    { from{opacity:0;transform:perspective(1400px) rotateY(65deg) scale(.97)} to{opacity:1;transform:perspective(1400px) rotateY(0) scale(1)} }
  @keyframes genPulse  { 0%,100%{transform:scaleX(.5);opacity:.45} 50%{transform:scaleX(1);opacity:1} }
  @keyframes shimmer   { 0%{background-position:200% 0} 100%{background-position:-200% 0} }
  @keyframes floatY    { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-9px)} }
  .anim-bookUp  { animation: bookUp .55s ease both }
  .anim-pageOut { animation: pageOut .3s ease forwards }
  .anim-pageIn  { animation: pageIn .32s ease both }
  .anim-visible { animation: bookUp .45s ease both }
  .cover-flip   { transform-origin:left center; animation:flipClose 1.5s cubic-bezier(.4,0,.2,1) forwards; backface-visibility:hidden }
  .pages-in     { animation:pagesIn 1.5s ease forwards }
  ...
  .book-lines   { background-image:repeating-linear-gradient(to bottom,transparent,transparent 27px,rgba(167,139,250,.14) 27px,rgba(167,139,250,.14) 28px) }
  .float-anim   { animation:floatY 3.2s ease-in-out infinite }
`

export default function StoryBook({ segments, done, brief, creativeNote, isGenerating, onNewStory, onViewHistory }) {
    ...
    useEffect(() => { endRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }) }, [segments])

    useEffect(() => {
        if (done && !bookOpen) {
            const t = setTimeout(() => {
                setIsOpening(true)
                setTimeout(() => { setBookOpen(true); setAnim('anim-visible') }, 1560)
            }, 600)
    ...
    const flip = (dir) => {
        ...
        setFlipping(true); setAnim('anim-pageOut')
        setTimeout(() => {
            setIdx(next); setAnim('anim-pageIn')
            setTimeout(() => { setFlipping(false); setAnim('anim-visible') }, 340)
        }, 310)
    }
    ...
    if (!done && isGenerating) return (
        <div style={{ maxWidth: 660, margin: '0 auto', padding: '40px 20px' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
                <div className="float-anim" style={{ display: 'inline-block', marginBottom: 14 }}>
                    <Px grid={WAND} size={9} />
                </div>
                <div style={{ fontFamily: "'Jersey 10',monospace", fontSize: 32, color: '#6D28D9', letterSpacing: '1.5px', marginBottom: 6 }}>
                    Writing {brief.child_name || 'the'} story...
                </div>
                <div style={{ fontFamily: "'DM Sans',sans-serif", color: '#9CA3AF', fontSize: 15, fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 5, marginBottom: 24 }}>
                {[0, 1, 2, 3, 4, 5].map(i => (
                    <div key={i} style={{
                        ...
                        animation: `genPulse 1.5s ease-in-out infinite`, animationDelay: `${i * .18}s`
                    }} />
                ))}
            </div>

            <div style={{
                background: '#F5F3FF', border: '1.5px dashed #C4B5FD', borderRadius: 20,
                padding: '28px 32px', minHeight: 200, fontFamily: "'DM Sans',sans-serif",
                fontSize: 16, lineHeight: 1.95, color: '#374151', fontWeight: 500
            }}>
                ...
                {segments.filter(s => s.type === 'image_loading').map((s, i) => (
                    <div key={i} style={{ marginTop: 14, marginBottom: 6 }}>
                        ...
                        <div style={{ fontFamily: "'Jersey 10',monospace", fontSize: 13, color: '#A78BFA', textAlign: 'center', marginTop: 6, letterSpacing: '1px' }}>
                            Painting illustration...
                        </div>
                    </div>
                ))}
                {segments.length === 0 && <span style={{ color: '#9CA3AF', fontStyle: 'italic' }}>The story is beginning...</span>}
                <div ref={endRef} />
            </div>
            ...
            {creativeNote && (
                <div style={{
                    ...
                    border: '1.5px dashed #FECDD3', borderRadius: 12, fontSize: 13,
                    color: '#9F1239', fontStyle: 'italic', fontWeight: 500, lineHeight: 1.6
                }}>
                    🎨 {creativeNote}
                </div>
            )}
    ...
    /* ── BOOK COVER (done but not yet opened) ── */
    if (done && !bookOpen) return (
        <div className="anim-bookUp" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '44px 20px' }}>
            ...
            <div style={{ position: 'relative', width: 240, height: 340, cursor: isOpening ? 'default' : 'pointer', marginBottom: 44 }}
                onClick={() => { if (!isOpening) { setIsOpening(true); setTimeout(() => setBookOpen(true), 1560) } }}>

                {/* page stack */}
                <div style={{
                    position: 'absolute', left: 8, top: 4, width: '100%', height: '100%',
                    background: '#F0EBE3', borderRadius: '4px 14px 14px 4px', boxShadow: '3px 5px 18px rgba(0,0,0,.1)'
                }} />
                <div style={{
                    position: 'absolute', left: 4, top: 2, width: '100%', height: '100%',
                    background: '#FBF7F2', borderRadius: '4px 14px 14px 4px'
                }} />

                {/* pages revealed during open */}
                {isOpening && (
                ...
                <div className={isOpening ? 'cover-flip' : ''}
                    ...
                    <div style={{
                        fontFamily: "'Jersey 10',monospace", fontSize: 20, color: '#EDE9FE',
                        textAlign: 'center', letterSpacing: '1.5px', lineHeight: 1.45
                    }}>
                        {brief.child_name || 'My'}'s<br />Storybook
                    </div>
                    <div style={{ width: 44, height: 1.5, background: 'rgba(196,181,253,.4)', borderRadius: 2 }} />
                    <div style={{
                        fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: '#A78BFA',
                        fontWeight: 600, letterSpacing: '.8px', textTransform: 'uppercase'
                    }}>
                        {brief.style} · ages {brief.age_group}
                    </div>
                    <div style={{
                        position: 'absolute', left: 14, top: 18, bottom: 18, width: 1.5,
                        background: 'rgba(167,139,250,.2)', borderRadius: 4
                    }} />
                </div>
            </div>
            ...
            {!isOpening ? (
                <button
                    onClick={() => { setIsOpening(true); setTimeout(() => setBookOpen(true), 1560) }}
                    style={{
    ...
    if (bookOpen && spreads.length > 0) return (
        ...
        minHeight: 460, borderRadius: '6px 20px 20px 6px',
        boxShadow: '-5px 0 0 #C4B5FD, 10px 12px 44px rgba(109,40,217,.12), -3px 4px 18px rgba(0,0,0,.07)'
    }}>
        ...
        <div style={{
            ...
            borderRight: '2px solid rgba(196,181,253,.3)',
            padding: '32px 24px',
            display: 'flex', flexDirection: 'column',
            alignItems: 'center', justifyContent: 'center', gap: 14
        }}>
            <div style={{
                fontFamily: "'Jersey 10',monospace", fontSize: 12, color: '#A78BFA',
                letterSpacing: '2px', textTransform: 'uppercase'
            }}>
                Page {idx + 1}
            </div>
            {spread?.image ? (
                <img src={spread.image} alt={`Illustration ${idx + 1}`}
                    style={{ width: '100%', borderRadius: 12, boxShadow: '0 4px 20px rgba(109,40,217,.14)', display: 'block' }}
            ...
            <div style={{ opacity: .45 }}><Px grid={CAT} size={4} /></div>
        </div>

        {/* SPINE */}
        <div style={{
            width: 16, flexShrink: 0,
            background: 'linear-gradient(to right,rgba(196,181,253,.5),rgba(237,233,254,.1))'
        }} />

        {/* RIGHT — text */}
        <div className="book-lines" style={{
            flex: 1, background: '#FFFDF7',
            borderRadius: '0 20px 20px 0', padding: '40px 32px 32px',
            fontFamily: "'DM Sans',sans-serif", fontSize: 17, lineHeight: 1.95,
            color: '#374151', fontWeight: 500, minHeight: 440, position: 'relative'
        }}>
            {spread?.text
            ...
            {creativeNote && idx === 0 && (
                <div style={{
                    ...
                    fontSize: 12, color: '#9F1239', fontStyle: 'italic', lineHeight: 1.6
                }}>
                    🎨 {creativeNote}
                </div>
            )}
            <div style={{
                ...
                fontFamily: "'Jersey 10',monospace", fontSize: 13, color: '#C4B5FD', letterSpacing: '1px'
            }}>
                {idx + 1}
            </div>
        </div>
    ...
    <div style={{
        ...
        <button onClick={() => flip('prev')} disabled={idx === 0 || flipping}
            style={{
                ...
                boxShadow: idx === 0 ? 'none' : '0 4px 0 #DDD6FE', transition: 'all .15s'
            }}>
            ← Prev
        </button>
        ...
        <button onClick={() => flip('next')} disabled={idx >= spreads.length - 1 || flipping}
            style={{
                fontFamily: "'Jersey 10',monospace", fontSize: 16, letterSpacing: '1px',
    ...
    /* ── FALLBACK — catches error/empty state ── */
    return (
